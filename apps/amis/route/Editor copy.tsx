...
@inject('store')
@observer
class EOCEditor extends React.Component<{store: IMainStore} 
& RouteComponentProps<{id: string}>
> {
    
    componentWillMount(){
        const index: number = 0;
        const page = this.props.store.pages[index];
        if (index !== currentIndex) {
            currentIndex = index;
            this.props.store.updateSchema(this.props.store.pages[index].schema);
        }
        ...
        apiRequest({
            ...
            // this.props. store.pages[index].updatePageInfo(schemaObject);
            count++;

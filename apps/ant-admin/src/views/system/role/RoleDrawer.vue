...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    watchEffect(() => {
      if (!roleDetails.value.permissions) {
        return
      }
      roleDetails.value.permissions?.forEach((item) => {
        allPermissions.value.forEach((permission) => {
          if (permission.impliedBy) {
            var flag = isImpliedByPermissionName(item, permission.impliedBy)
            if (flag) {
              permission.checkable = false
            }
          }
        })
      })
    ...
    function isImpliedByPermissionName(
      impliedByPermissionName: string,
      permissions: PermissionTreeItem[],
    ): boolean {
      //遍历权限列表

      return permissions.some((permission) => {
        if (permission.name === impliedByPermissionName) {
          return true
        } else if (permission.impliedBy) {
          return isImpliedByPermissionName(
            impliedByPermissionName,
